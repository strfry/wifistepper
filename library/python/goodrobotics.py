    ...
    def _rxworker(self):
        while True:
            ...
            if p_type == self._TYPE_ERROR:
                ...
                (d_product, d_model, d_swbranch, d_version, d_hostname, d_chipid, self.enabled_std, self.enabled_crypto, self.nonce) = struct.unpack(self._PACK_HELLO, self.sock.recv(struct.calcsize(self._PACK_HELLO)))
...
class WifiStepper:
    ...
    def runqueue(self, queue, target = None):
        return self.__comm.cmd_runqueue(self._target(target), queue)
